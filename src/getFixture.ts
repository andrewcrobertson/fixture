...
export const getFixture = async (page: Page, round: number) => {
  ...
  for (let i = 0; i < teamConfig.length; i++) {
    ...
    try {
      await page.goto(
        `https://websites.sportstg.com/comp_info.cgi?round=${round}&a=ROUND&client=1-722-0-${teamX.compId}-0`
      );

      const name = await page.$$(
        `//select[@name="round"]/option[@value="${round}"]`
      );
      ...
      for (let j = 0; j < gameRows.length; j++) {
        ...
        const timeElement = await gameRow.$(
          '//div[contains(@ng-bind-html, "::m.Time")]'
        );
        const venueElement = await gameRow.$(
          '//a[contains(@class, "venuename")]'
        );
        ...
        if (homeTmp === teamX.name || awayTmp === teamX.name) {
          ...
          const venue =
            venueTmp === "Bayswater Junior Oval" ? "Home" : venueTmp;
          postContent.push(
            `${teamX.bullet} ${
              teamX.team
            } Vs ${oppositionTeam} @ ${venue} - ${timeTmp
              .replace("&nbsp;", "")
              .toLowerCase()}`
          );
        }
      ...
      for (let j = 0; j < byeRows.length; j++) {
        ...
        if (team === teamX.name)
          postContent.push(`${teamX.bullet} ${teamX.team} BYE`);
      }
    } catch {
